...
void print_wav_information(const char* fn, int format, int channels,
                           int sample_rate, int bits_per_sample, int length) {
  cout << "=====================================" << endl
       << fn << " information:" << endl
       << "format: " << format << endl
...
void print_progress(int current, int total) {
  ...
  cout << ">";
  for (int i = progress; i < p_bar_length; ++i) cout << " ";
  cout << "|";
  cout << "\r";
}
...
int main(int argc, char* argv[]) {
  ...
  int res = wav_get_header(h_ref, &ref_format, &ref_channels, &ref_sample_rate,
                           &ref_bits_per_sample, &ref_data_length);
  if (!res) {
    cerr << "get ref header error: " << res << endl;
    return -1;
  }
  int ref_samples = ref_data_length * 8 / ref_bits_per_sample;
  print_wav_information(argv[1], ref_format, ref_channels, ref_sample_rate,
                        ref_bits_per_sample, ref_data_length);

  res = wav_get_header(h_rec, &rec_format, &rec_channels, &rec_sample_rate,
                       &rec_bits_per_sample, &rec_data_length);
  if (!res) {
    cerr << "get rec header error: " << res << endl;
    return -1;
  }
  int rec_samples = rec_data_length * 8 / rec_bits_per_sample;
  print_wav_information(argv[2], rec_format, rec_channels, rec_sample_rate,
                        rec_bits_per_sample, rec_data_length);

  if (ref_format != rec_format || ref_channels != rec_channels ||
  ...
  std::unique_ptr<EchoControl> echo_controler =
      aec_factory.Create(ref_sample_rate, ref_channels, rec_channels);
  std::unique_ptr<HighPassFilter> hp_filter =
      std::make_unique<HighPassFilter>(rec_sample_rate, rec_channels);
  ...
  std::unique_ptr<AudioBuffer> aec_linear_audio = std::make_unique<AudioBuffer>(
      kLinearOutputRateHz, config.num_channels(), kLinearOutputRateHz,
      config.num_channels(), kLinearOutputRateHz, config.num_channels());

  AudioFrame ref_frame, aec_frame;

  void* h_out = wav_write_open(argv[3], rec_sample_rate, rec_bits_per_sample,
                               rec_channels);
  void* h_linear_out = wav_write_open("linear.wav", kLinearOutputRateHz,
                                      rec_bits_per_sample, rec_channels);

  int samples_per_frame = sample_rate / 100;
  int bytes_per_frame = samples_per_frame * bits_per_sample / 8;
  int total = rec_samples < ref_samples ? rec_samples / samples_per_frame
                                        : rec_samples / samples_per_frame;
  ...
  while (current++ < total) {
    ...
    ref_frame.UpdateFrame(0, reinterpret_cast<int16_t*>(ref_tmp),
                          samples_per_frame, sample_rate,
                          AudioFrame::kNormalSpeech, AudioFrame::kVadActive, 1);
    aec_frame.UpdateFrame(0, reinterpret_cast<int16_t*>(aec_tmp),
                          samples_per_frame, sample_rate,
                          AudioFrame::kNormalSpeech, AudioFrame::kVadActive, 1);
    ...
    echo_controler->ProcessCapture(aec_audio.get(), aec_linear_audio.get(),
                                   false);
    ...
    aec_frame.UpdateFrame(0, nullptr, kLinearOutputRateHz / 100,
                          kLinearOutputRateHz, AudioFrame::kNormalSpeech,
