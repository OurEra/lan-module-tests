...
unsigned int pskCallback(SSL *ssl, const char *hint, char *identity,
...
int main(int argc, char *argv[]) {
  ...
  if (readSessionFile) {
    ...
    if (debuglevel > 2)
      fprintf(stderr, "Reading session from %s\n", readSessionFile);
  }
  ...
  if (alpn_protos.length() > 0) {
    ...
    SSL_set_alpn_protos(ssl, (unsigned char *)alpn_protos.data(),
                        alpn_protos.length());
  }
#endif

  SSL_set_connect_state(
      ssl);  // Can't fail, and not needed if we call SSL_connect

  CHECK(sslConnect(ssl) == SSL_OK);  // Perform initial handshake synchronously

  timespec t2;
  clock_gettime(CLOCK_MONOTONIC_RAW, &t2);
  long cost2_1 = (t2.tv_sec - t1.tv_sec) * 1000 + (t2.tv_nsec - t1.tv_nsec) / 1000000;
  printf("cost2_1 %ldms\n", cost2_1);
  ...
  long cost3_2 = (t3.tv_sec - t2.tv_sec) * 1000 + (t3.tv_nsec - t2.tv_nsec) / 1000000;
  printf("cost3_2 %ldms\n", cost3_2);
