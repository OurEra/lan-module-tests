...
void sigint_handler(int) { terminated = true; }
...
void describeCertificate(int i, X509 *cert) {
  ...
  X509_NAME_print_ex_fp(stderr, X509_get_subject_name(cert), 0,
                        XN_FLAG_ONELINE);
  fprintf(stderr, "\n");
...
void describeCertificates(SSL *ssl, bool isServer) {
  // Resumed sessions don't necessarily have chains (not included in session
  // ticket)
  ...
  if (cert == NULL) {
    ...
    X509_free(cert);
    STACK_OF(X509) *certs =
        SSL_get_peer_cert_chain(ssl);  // We don't have to free this apparently
...
void describeSession(SSL *ssl) {
  ...
  }
  {
    ...
    char *s = hex_to_string(session->sid_ctx, session->sid_ctx_length);
...
void sslCleanup() {
  ...
  // Maddeningly, 64 bytes still remains (see
  // https://bugs.launchpad.net/percona-server/+bug/1205196)
  // ENGINE_cleanup();
...
int sslConnect(SSL *ssl) {
  while (true) {
    ...
    if (ret >= 0) {
      ...
      if ((SSL_get_error(ssl, ret) != SSL_ERROR_SYSCALL || errno != EAGAIN) &&
          ret < 0) {
        return ret;
      }
    }
...
// should be closed.
int verifyCallback(int preverify_ok, X509_STORE_CTX *ctx) { return 1; }
...
bool sslLoop(SSL *ssl, int fd, bool isserver, bool verify, bool waitforpeer) {
  ...
  while (!terminated || SSL_renegotiate_pending(ssl)) {
    ...
    if (!closed && !write_pending &&
        (!isserver || !SSL_renegotiate_pending(ssl))) {
      FD_SET(0, &rfds);
    }
    if (write_pending) {
      if (write_wantread) {
