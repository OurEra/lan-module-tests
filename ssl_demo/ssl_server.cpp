...
void setupClientVerification(SSL_CTX *ctx, const char *caFile) {
  ...
  // verifyCallback); // No return value
}
...
void setupDH(SSL_CTX *ctx) {
  ...
  if (fp == NULL) {
    ...
    int generator = 5;
    if (debuglevel > 1)
      fprintf(stderr, "DH_generate_parameters(%d,%d)...\n", prime_len,
              generator);
    dh = DH_new();
    ...
    if (debuglevel > 1)
      fprintf(stderr, "Reading DH params from %s\n", dhparamfile);
...
unsigned int pskServerCallback(SSL *ssl, const char *identity,
  ...
  if (debuglevel > 2)
    fprintf(stderr, "PSK callback for identity '%s'\n", identity);
...
int main(int argc, char *argv[]) {
  ...
  // For actual security, 'ECDH:RSA:ALL !DH !RC4 !DES !SRP !PSK !aNULL !eNULL'
  // passes the SSL Labs test. Allow ADH/AECDH/SRP as well as NULL for testing
  ...
  CHECK(SSL_CTX_set_session_id_context(ctx, (const unsigned char *)context,
